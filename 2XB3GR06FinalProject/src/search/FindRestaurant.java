...
//			if (allRestaurantViolations.size() != allRestaurantInspections.size()) {
//				found = false;
//			}
		}
		else
	...
	//demonstration of how to use this class
	public static void main(String args[]) throws IOException{
		
		while (true) {
		Scanner s = new Scanner(System.in);  // Create a Scanner object
		System.out.println("Enter Restaurant");
		String restaurantSearch = s.nextLine().toUpperCase();
		File violationsFile = new File("Clean Restaurant and Market Health Violations.csv");
		File inspectionsFile = new File("Clean Restaurant Inspections.csv");
		FindRestaurant fr = new FindRestaurant(restaurantSearch, violationsFile, inspectionsFile);
		boolean found1= fr.getFound();
		
		if (found1 == true) {
			
			for (int i = 0; i < fr.getViolationArrayList().size(); i ++) {
				System.out.print(fr.getInspectionArrayList().get(i).get(0).getFacilityName() + ", ");
				System.out.println(fr.getInspectionArrayList().get(i).get(0).getFacilityAddress());
	
				System.out.println("VIOLATIONS:");
				System.out.println("Date, Service Type, Violation Description");
				boolean noViolations = true;
				for (int j = 0; j < fr.getViolationArrayList().get(i).size(); j ++) {
					System.out.println(fr.getViolationArrayList().get(i).get(j).getActivityDate()
				...
				for (int k = 0; k < fr.getInspectionArrayList().get(i).size(); k ++) {
					System.out.println(fr.getInspectionArrayList().get(i).get(k).getActivityDate());
				}
				System.out.println("SafeFoodsScore: " + Score.safeFoodScore(fr.getViolationArrayList().get(i), fr.getInspectionArrayList().get(i)));
			}
		}
