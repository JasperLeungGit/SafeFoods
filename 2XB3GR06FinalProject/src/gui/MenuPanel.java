	...
	private void output(String restaurant) throws IOException {
		...
		if (found1 == true) {
			...
			for (int i = 0; i < fr.getViolationArrayList().size(); i ++) {
				...
				details += fr.getInspectionArrayList().get(i).get(0).getFacilityAddress() + newLine;
				...
				if (noViolations)
					...
					details += "Date, Service Type, "
							+ "Violation Description" + newLine;
					for (int j = 0; j < fr.getViolationArrayList().get(i).size(); j ++) {
						details += fr.getViolationArrayList().get(i).get(j).getActivityDate()
				...
				details += "INSPECTIONS:" + newLine;
				details += "Date, Grade" + newLine;
				
				for (int k = 0; k < fr.getInspectionArrayList().get(i).size(); k ++) {
					details += fr.getInspectionArrayList().get(i).get(k).getActivityDate() 
							+ ", " + fr.getInspectionArrayList().get(i).get(k).getGrade() 
							+ newLine;
				}
				
				details += "SafeFoodsScore: " + Score.safeFoodScore(fr.getViolationArrayList().get(i), 
